...
knitr::include_graphics("imag/03_Miel_en_Mexico.png")
...
<p align="center"> **Cuadro 3.1. Composición nutricional característica de la miel** </p>

<br/>

```{r tab0008, echo=FALSE, out.width= "60%",warning=FALSE}
library(kableExtra)
tab0008 <- read.csv("tabs/nutrientes_miel.csv", encoding = "utf8")
...
En el caso de la miel de abeja la [Norma Oficial Mexicana NOM-145-SCFI-2001. Información comercial-Etiquetado de miel en sus diferentes presentaciones](http://www.colpos.mx/bancodenormas/noficiales/NOM-145-SCFI-2001.PDF){target="_blank"}, sirve de referente legal para complementar lo solicitado en la NOM-051-SCFI/SSA1-2010.
...
La [NMX-F-036-981. Miel de Abeja. Especificaciones](https://www.colpos.mx/bancodenormas/nmexicanas/NMX-F-036-1981.PDF){target='_blank'}, clasifica la miel en las siguientes modalidades:
...
Por su parte, la [Norma Oficial Mexicana NOM-004-SAG/GAN-2018, Producción de miel y especificaciones](http://dof.gob.mx/nota_detalle.php?codigo=5592435&fecha=29/04/2020){target='_blank'}, establece que la miel puede designarse con el nombre de la región geográfica o topográfica, si ha sido producida exclusivamente en el área a que se refiere la denominación.
...
* Declarar que los productos son útiles para prevenir, aliviar, tratar o curar una enfermedad, trastorno o estado fisiológico ([Reglamento de la Secretaría de Salud en Materia de Publicidad](http://www.salud.gob.mx/unidades/cdi/nom/compi/rlgsmp.html#:~:text=REGLAMENTO%20de%20la%20Ley%20General,Salud%20en%20Materia%20de%20Publicidad.&text=ART%C3%8DCULO%201.,la%20Ley%20General%20de%20Salud){target='_blank'}).
...
En el nuevo régimen que se maneja en la [Modificación a la Norma Oficial Mexicana NOM-051-SCFI/SSA1-2010, Especificaciones generales de etiquetado para alimentos y bebidas no alcohólicas preenvasados-Información comercial y sanitaria](https://www.dof.gob.mx/2020/SEECO/NOM_051.pdf){target="_blank"}, sobre la información nutrimental complementaria y las leyendas precautorias, la miel queda exceptuada, por lo que no es necesario incluir ninguno de los siguientes sellos: 

<br/>

```{r i_etiq, fig.align='center', fig.cap="**Figura 3.2. Sellos para la identificación de alimentos con alto contenido de calorías o azúcares.**", echo=FALSE, out.width = "40%", cache=TRUE}

knitr::include_graphics("imag/etiq.png")
...
```{r i_tantoyuca, fig.align='center', fig.cap="**Figura 3.3. Vino de miel Tantoyuca y sus premios en el concurso mundial de Bruselas, 2017 y 2018.**", echo=FALSE, out.width = "80%", cache=TRUE}

knitr::include_graphics("imag/tantoyucan.png")
...
La misma [Ley General de Salud](http://www.salud.gob.mx/cnts/pdfs/LEY_GENERAL_DE_SALUD.pdf){target="_blank"} menciona al [Acuerdo por el que se determinan las sustancias prohibidas y restringidas en la elaboración de productos de perfumería y belleza](http://dof.gob.mx/nota_detalle.php?codigo=5143790&fecha=21/05/2010){target="_blank"}, mismo que debe revisarse una vez que se decida fabricar algún producto cosmético. Así mismo en cuestión de etiquetado la [Norma Oficial Mexicana NOM-141-SSA1/SCFI-2012, Etiquetado para productos cosméticos preenvasados. Etiquetado sanitario y comercial](http://dof.gob.mx/nota_detalle.php?codigo=5269348&fecha=19/09/2012){target='_blank'}, da las especificaciones para ello.
...
4. **Requisitos y trámites ante las dependencias gubernamentales:**
    ...
    Solicitud de Certificado Zoosanitario de Exportación para productos y subproductos de origen animal ante SENASICA (AGRICULTURA).
...
```{r i0_3_20_01, fig.align='center', fig.cap="**Figura 3.4. Abeja extrayendo néctar**", echo=FALSE, out.width = "80%", cache=TRUE}
...
library(DT)
tab0010 <- read.csv("tabs/Ranking_tab_1_s.csv", encoding = "utf8")
...
<p align="center"> **Cuadro 3.2. Posición que ocuparon las entidades por nivel de producción de miel de abeja, 2003-2019** </p>
...
#### **Instituto Nacional de Investigaciones Forestales, Agrícolas y Pecuarias (INIFAP), perteneciente a la Secretaría de Agricultura y Desarrollo Rural (AGRICULTURA).**
...
<video width="600" height="400" controls muted>
    <source src="media/laboratorio_analisis_propoleos.mp4?rel=0" type="video/mp4">
</video>
...
<video width="600" height="400" controls muted>
    <source src="media/laboratorio_analisis_propoleos_unam_global.mp4?rel=0" type="video/mp4">
</video>
...
```{r i0_3_28_01, fig.align='center', fig.cap= "**Figura 3.5. Inseminación Instrumental en Abejas, elemento importante para la certificación.**", echo=FALSE, out.width = "80%", cache=TRUE}
 #par(mfrow = c(2, 2),
...
<p align="center"> **Cuadro 3.3. Criaderos de Abejas Reina F1 certificados durante 2017-2018 (Certificación genética y sanitaria).** </p>

<br/>
 

```{r tab001, echo=FALSE }
tab0001 <- read.csv("tabs/t001.csv", encoding = "utf8")
...
<p align="center"> **Cuadro 3.4. Criaderos de Abejas Reina Progenitora certificados durante 2017-2018 (Certificación Genética y Sanitaria).** </p>
...
tab0002 <- read.csv("tabs/t0002.csv", encoding = "utf8")
...
<p align="center"> **Cuadro 3.5. Productores de Núcleos certificados durante 2017-2018. (Certificación sanitaria).** </p>
...
tab0003 <- read.csv("tabs/t0003.csv", encoding = "utf8")
