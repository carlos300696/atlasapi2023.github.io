...
knitr::include_graphics("imag/02_Abejas_Apicultura.png")
...
[AGRICULTURA (2018). _Buenas Prácticas Pecuarias en la producción de Producción de Miel._ ](pdfs/Manual_BPP_en_la_Producci_n_primaria_de_Miel_octubre_2018.pdf){target="_blank"}
...
### **2.6.1. Programa de Fomento Ganadero de AGRICULTURA en Apoyo a la Apicultura**
...
Bajo ese contexto, las reglas de operación de los programas de AGRICULTURA se integran con una estructura dirigida a desarrollar y potencializar la productividad y eficiencia de los productores del sector agropecuario, pesquero y acuícola; definen los diferentes tipos de apoyo que se otorgan en efectivo o en especie, señalando montos o unidades a entregar por beneficiario.  
...
<p align="center"> **Cuadro 2.1. Asociaciones, cámaras, instituciones y publicaciones relacionadas con la apicultura.** </p>

```{r tab2_10_1, echo=FALSE, warning=FALSE }
library(kableExtra)
tab2_10_1 <- read.csv("tabs/asociaciones_api2.csv", header = F, encoding = "utf8")
names(tab2_10_1) <- c("Nombre", "Dirección")
...
<p align="center"> **Cuadro 2.2. Colmenas utilizadas en polinización durante el 2008.** </p>

```{r tab0x11, echo=FALSE }
library(kableExtra)
tabtab0x11 <- read.csv("tabs/poliniza_colm.csv", encoding = "utf8")
names(tabtab0x11) <- c( "DELEGACIÓN AGRICULTURA", "CULTIVOS", "COLMENAS")
...
[Delegación SADER Michoacán. (2017). *Destacada la Actividad de la Apicultura, Señala SADER Michoacán.*  Boletín de prensa, mayo.](https://www.gob.mx/agricultura%7Cmichoacan/articulos/destacada-la-actividad-de-la-apicultura-senala-sagarpa-michoacan){target="_blank"}
...
<p align="center"> **Cuadro 2.3. Principales enfermedades de las crías de abejas en México.** </p>

```{r tab0004, echo=FALSE }

library(kableExtra)
tab0004 <- read.csv("tabs/cua_enferm_1s.csv", encoding = "utf8")
...
<p align="center"> **Cuadro 2.4. Principales enfermedades de las abejas adultas en México.** </p>

```{r tab0005, echo=FALSE }

tab0005 <- read.csv("tabs/cua_enferm_2s.csv", encoding = "utf8")
...
<p align="center"> **Cuadro 2.5. Principales plagas de las abejas en México.** (tabs/cua_plaga_4s.csv) </p>

```{r tab0006, echo=FALSE }

tab0006 <- read.csv("tabs/cua_plaga_4s.csv", encoding = "utf8")
...
<p align="center"> **Cuadro 2.6. Otros tipos de plagas de las abejas.** </p>

```{r tab0007, echo=FALSE }

tab0007 <- read.csv("tabs/cua_otras_6s.csv", encoding = "utf8")
...
### **2.12.4 Enfermedades Virales de las Abejas**

Por: M.V.Z. Elizabeth Bacilio López, FES Cuatitlán, UNAM.

<br>

**Enfermedades y Plagas de las Colmenas.**

Aristóteles (384-322 a.C.) describió algunas alteraciones, y Virgilio y Plinio hacen referencia a algunas en los inicios del primer milenio. Ninguna de sus descripciones es suficiente para identificar las alteraciones con certeza. No obstante, ponen claramente de manifiesto que las abejas de entonces eran en gran medida parecidas a las de hoy día y las enfermedades que hoy reciben “nombre” existían en la antigüedad. 

Las abejas son susceptibles de sufrir enfermedades causadas por bacterias (como *Loque americana* y *Loque europea*), virus (como el *Virus de la Parálisis Aguda Israelí*), hongos (como *Nosema spp.*) y parásitos (como el ácaro de *Varroa spp.*), además de parásitos mayores como la polilla (*Galleria mellonella*) o menores como el piojo (*Braula coeca*). Su resistencia a los factores adversos es mayor si se encuentran en óptimo estado sanitario y de nutrición.

Algunas de estas enfermedades están inscritas en: [*"Enfermedades, infecciones e infestaciones de la Lista de la Organización Mundial de Sanidad Animal (OIE) en vigor en 2020"*](https://www.oie.int/es/sanidad-animal-en-el-mundo/enfermedades-de-la-lista-de-la-oie-2020/){target="_blank"} y en el [*“Acuerdo mediante el cual se dan a conocer en los Estados Unidos Mexicanos las enfermedades y plagas exóticas y endémicas de notificación obligatoria de los animales terrestres y acuáticos (2018)”*](http://www.dof.gob.mx/nota_detalle.php?codigo=5545304&fecha=29/11/2018#:~:text=ACUERDO%20mediante%20el%20cual%20se,los%20animales%20terrestres%20y%20acu%C3%A1ticos){target="_blank"}. En el cuadro 2.7 se presenta un comparativo de las enfermedades de las abejas que en ellos se mencionan.

<br/>

<p align="center"> **Cuadro 2.7. Comparativo de las clasificaciones de enfermedades de las abejas registradas en la _Lista de la OIE_ y en el _Acuerdo de México (2018)_.** </p>

| **Lista de la OIE** | **Acuerdo de México (2018)** |
| -------------------| --------------------- |
| <br> Infestación de las Abejas Melíferas por *Tropilaelaps* | Grupo 1\* <br> I. Infestación de las Abejas Meliferas por *Tropilaelaps* (*Tropilaelaps clareae*, *T. koenigerum*, *T. mercedesae*, *T. thaii*) |
| <br> Loque Americana | Grupo 1\* <br> II. Loque Americana (*Paenibacillus larvae*) |
| <br> Varroosis | Grupo 2\*\* <br> I. Infestación por *Varroa* / Varroasis (*Varroa destructor*, *V. jacobsoni*, *V. underwoodi*, *V. rinderi*) |
| <br> Acarapisosis | Grupo 3\*\*\* <br> I. Acarapisosis (*Acarapis woodi*) |
| <br> Infestación por Escarabajo de las colmenas (*A. tumida*) | Grupo 3\*\*\* <br> II. Infestación por el Pequeño Escarabajo de la Colmena (*Aethina tumida*) |
| <br> Loque Europea | Grupo 3\*\*\* <br> III. Loque Europea (*Melissococcus plutonius*) |
| <br> Nosemosis de las abejas melíferas | Grupo 3\*\*\* <br> IV. Nosemosis (*Nosema apis* y *Nosema ceranae*) |
| | |

  \* Grupo 1: Enfermedades y plagas exóticas de notificación inmediata obligatoria.  
 \*\* Grupo 2: Grupo 2: Enfermedades enzoóticas transmisibles de notificación inmediata obligatoria.  
\*\*\* Grupo 3: Enfermedades enzoóticas de notificación mensual obligatoria.

<br>

En lo que corresponde a sanidad apícola, se han hecho muestreos en algunos estados de la República Mexicana para detectar agentes patógenos como *Varroa destructor* y *Nosema spp*. y *Virus de la Celda Real Negra (BQCV)*, *Virus de las Alas Deformes (DWV)*, *Virus de Cachemira (KBV)*, *Virus de la cría ensacada (SBV)*, *Virus de la Parálisis Crónica (CBPV)*, *Virus de la Parálisis Aguda (ABPV)* y *Virus de la Parálisis Aguda Israelí (IAPV)* de las abejas, aunque cabe mencionar que no se ha puesto mucha atención en las enfermedades virales y la relación que estas tienen con la presencia de ácaros como *Varroa destructor*, y del microsporideo *Nosema spp.* 

Las poblaciones de abejas se ven repetidamente afectadas por enfermedades y parásitos que con frecuencia se difunden y actúan con carácter epidémico.

<br>

**Virus de las abejas.**

Los virus son microorganismos estructuralmente sencillos, algunos poseen una estructura proteica que contiene una o más moléculas de ácido desoxirribonucleico (ADN) o de ácido ribonucleico (ARN). Se replican en las células animales y vegetales.
Cuando un virus infecta a una célula emplea los materiales de la célula para hacer copias de sí mismo. Esto puede continuar sin muchos cambios aparentes para la célula, mientras permanezca vivo el organismo al que pertenece; aunque generalmente las células infectadas padecen lesiones, mueren y se desintegran, liberando muchísimas partículas víricas infectivas. Todas las formas de vida son atacadas por los virus incluídas las abejas, las cuales son infectadas por una amplia variedad (cuadro 2.8). 

<br/>

<p align="center"> **Cuadro 2.8. Virus que afectan a las abejas.** </p>

<center>

| **Huésped** | **Virus** |
| -------------------| --------------------- |
| *Apis mellífera* | Parálisis Crónica (VPC) <br> Asociado a la Parálisis Crónica <br> Alas Nubladas <br> Filamentoso <br> X <br> Y <br> Cachemira (estirpes australianas) <br> Egipto <br> Cría Sacciforme <br> Arkansas <br> Parálisis Lenta (SBPV) <br> Celda Real Negra <br> Parálisis Aguda (VPA) <br> Paralisis Aguda Israelí (VPAI) <br> Virus de *Varroa destructor* |
| *Apis cerana* | Cría Sacciforme (estirpe Thai) <br> Cachemira (estirpe india) <br> Iridiscente (AIV) <br> Kakugo |
| | |

***Modificada de Bailey (1984), Bailey y Ball (1991) y Bromenshenk _et al._ (2010).***

</center>

<br/>

La disminución alarmante de las colonias de abejas *Apis mellifera* en la última década ha conducido la atención y la investigación de diferentes patógenos que las afectan, incluyendo los virus. Numerosos estudios sugieren que los patógenos nuevos o emergentes desempeñan un papel muy importante en la disminución de su salud. En México existen pocos estudios que ayuden a conocer el estatus sanitario de las abejas, ya que los métodos de detección de los virus suelen ser costosos y no se tienen protocolos establecidos, además de que las enfermedades virales no son consideradas de reporte obligatorio en México.

Hasta la mitad del siglo pasado el único virus conocido que afectaba a las abejas era el virus de la Cría Sacciforme. Actualmente, el número de virus identificados en el género *Apis* se eleva a más de 18, entre los que se encuentra el Virus de la Parálisis Aguda Israelí. 

<br>

**Enfermedades virales paralizantes de las abejas.**

Se trata de un conjunto de enfermedades víricas o síndromes que presentan signos semejantes y pueden tener dos manifestaciones diferentes, uno se caracteriza por provocar la muerte de las abejas en poco tiempo. A éste se le denomina parálisis aguda, causada por el VPA y el VPAI; el otro produce un temblor paralítico por algunos días después de haber presentado los primeros signos, esta se denomina parálisis crónica, causada por el VPC, el Virus asociado a la Parálisis Crónica y el SBPV (cuadro 2.9) .

<br/>

<p align="center"> **Cuadro 2.9. Características de los virus causantes de parálisis en las abejas.** </p>

<center>

| **Virus** | **Diámetro (nm)** | **Ácido Nucleico <br> y Pesos moleculares (x 10^-6^)** |
| -------------------| --------------------- | ----------------------- |
| Parálisis crónica (VPC) | 20 x 30-60 | ARN 1.35, 0.9, 0.35 | 
| Virus asociado a la Parálisis Crónica | 17 | ARN 0.35 | 
| Parálisis aguda (VPA) | 30 | ARN ND\* |
| Parálisis lenta (SBPV) | 30 | ARN ND\* |
| Parálisis Aguda Israelí (VPAI) | 27-30 | ARN ND\* |
| | |
\* No Determinado  
***Modificada de Bailey (1984), Bailey & Ball (1991) y Boncristiani _et al._ (2013).***
...
La transmisión de los virus en las abejas parece implicar las vías horizontales y verticales como se muestra en la figura 2.7. Los virus pueden infectar diferentes estadios de las abejas de la misma generación por transmisión horizontal a través de los siguientes medios: por alimentos, fecal-oral, venérea (sexual), y/o por vectores.

Algunos virus también se transmiten verticalmente desde la reina infectada a su descendencia. Ambas vías de transmisión se cree que son las estrategias de supervivencia importantes para la persistencia y el establecimiento de los virus en la población de las abejas. 

<br/>

```{r i_mecanismos, fig.align='center',fig.height= 6, fig.width= 12, echo=FALSE, cache=TRUE, fig.cap="**Figura 2.7. Representación esquemática de las vías de transmisión de los virus en las abejas.**", out.width = "80%"}

knitr::include_graphics("imag/mecanismos.png")

```

<center>

**Fuente: Imagen modificada de Shimanuki y Knox (2000), Shen _et al._ (2005), y Chen & Sied (2007).**
...
• Martínez P.J.F., Alcalá E.K.I., Leal H.M., Vivas R.J.A. y Martínez A.E. (2011) *Manual Varroosis. Prevención de Varroosis y Suplementación*. INIFAP-SAGARPA. México.

• Jean-Prost P. y Médori P. (2007). *Apicultura. Conocimiento de la abeja. Manejo de la colmena*. Editorial Mundi- Prensa, 4ª ed. España.

• Bailey L. (1984) *Patología de las abejas*. 1ª ed., Acribia, España.

• Bailey L. y Ball B. V. (1991) *Honey Bee Pathology*. 2ª ed., Academic Press, San Diego CA.

• Chen Y. P., Evans J., Feldlaufer M. (2006). *Horizontal and vertical transmission of viruses in the honeybee, Apis mellifera*. Journal of Invertebrate Pathology, 92: 152-159.

• Chen Y. P. y Siede R. (2007). *Honey Bee Viruses*. Advances in Virus Research, 70: 33-80.

• Cornejo L. G. y Rossi C. O. (1975). *Enfermedades de las abejas. Su profilaxis y Prevención*. 2ª ed., Hemisferio, Argentina.

• Izquierdo R. (2000). *Ingeniería genética y transferencia genética*. Pirámide, España.

• Mc Gregor S. E. (1992). *Apicultura*. 8ª reimp. Limusa-Noriega, México.

• Dainat B. y Gauthier L. (2013). *Virus de la Parálisis Aguda (VPA)*. En: Leal H. M. y Arachevaleta V. M. E. Editores. (2013). *Principales Enfermedades de las Abejas en México*. SAGARPA-INIFAP, México.

• Benjeddou M., Leat N., Allsopp M. y Davison S. (2001). *Detection of Acute Bee Paralysis Virus and Black Queen Cell Virus from Honeybees by Reverse Transcriptase PCR*. Applied and Environmental Microbiology, 67 (5): 2384–2387.

• Shen M., Cui L., Ostiguy N. y Cox-Foster D. (2005). *Intricate transmission routes and interactions between picoARN-like viruses (Kashmir bee virus and sacbrood virus) with the honeybee host and the parasitic varroa mite*. Journal of General Virology, 86: 2281–2289.

• Boncristiani H. F., Evans J. D., Chen Y., Pettis J., Murphy C., *et al.* (2013). *In Vitro Infection of Pupae with Israeli Acute Paralysis Virus Suggests Disturbance of Transcriptional Homeostasis in Honey Bees (Apis mellifera)*. PLoS ONE, 8 (9): e73429.

• OIE. [Información sobre las enfermedades de los animales acuáticos y terrestres](https://www.oie.int/es/sanidad-animal-en-el-mundo/enfermedades-de-los-animales/){target="_blank"}

• [*“Acuerdo mediante el cual se dan a conocer en los Estados Unidos Mexicanos las enfermedades y plagas exóticas y endémicas de notificación obligatoria de los animales terrestres y acuáticos (2018)”*](http://www.dof.gob.mx/nota_detalle.php?codigo=5545304&fecha=29/11/2018#:~:text=ACUERDO%20mediante%20el%20cual%20se,los%20animales%20terrestres%20y%20acu%C3%A1ticos){target="_blank"}. DOF: 29/11/2018
